  ...
  const handleFetch = async () => {
      ...
      swal.fire({
        ...
        html: `
          <ul style="text-align:left">
            ...
            ${!front ? "<li>Aadhaar Front is required</li>" : ""}
    ...
    try {
      const res = await api.post(
        "api/fetchAadhaarOcrV2Controller",
        formData
      );
      ...
      if (apiData?.code === "1014") {
        swal.fire({
          title: "Success",
          html: apiData?.message || "OCR completed",
          icon: "success",
        });
      } else {
        swal.fire({
      ...
      content: [
...

        {
          table: {
            widths: ["40%", "60%"],
            body: [
              ["Aadhaar Number", safe(d.document_id)],
              ["Name", safe(d.name)],
              ["Guardian", safe(d.guardian_name)],
              ["Gender", safe(d.gender)],
              ["DOB", safe(d.date_of_birth)],
              ["Contact", safe(d.contact_number)],
      ...
      <Col md={12}>
        ...
        <Card body className="mb-4">
          <Row>
            ...
            <Col md={4}>
              <Form.Label>Aadhaar Front <Required /></Form.Label>
              <Form.Control
                type="file"
                accept=".jpg,.jpeg,.png,.pdf"
                onChange={(e) => setFront(e.target.files[0])}
