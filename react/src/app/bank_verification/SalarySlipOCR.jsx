  ...
  const handleFetch = async () => {
      ...
      swal.fire({
        ...
        html: `
          <ul style="text-align:left">
            ${!file ? "<li>Salary Slip required</li>" : ""}
    ...
    try {
      const res = await api.post(
        "api/fetchSalarySlipOcrController",
        formData
      );

      const full = res.data?.data;
      // console.log("full" , res.data?.data)
...
const exportPdf = () => {
  ...
  pdfMake
    .createPdf(doc)
    .download(`SALARY_SLIP_OCR_${fileNo}.pdf`);
};
        ...
        {result && (
          <Card body>
            <div className="d-flex justify-content-between">
              <h5>
                Result{" "}
                <Badge bg={getBadgeVariant()}>{code}</Badge>
              </h5>
