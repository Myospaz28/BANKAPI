...
export default function FetchPanEssentials() {
  ...
  const handleFetch = async () => {
    ...
    try {
      const res = await api.post('api/pan/fetchEssentials', {
        usr_ser_id,
        pan_number: pan,
        file_no: fileNo,
        consent: 'Y',
...
const code = apiData?.code;
  ...
  swal.fire(
    'Failed',
    apiData?.message || 'PAN fetch failed',
    'error',
  );
  return;
}
  ...
  const doc = {
  content: [
    { text: 'PAN Essentials Verification Report', style: 'header' },
...
['Date of Birth', d.date_of_birth || '-'],
    ...
    pdfMake.createPdf(doc).download(`PAN_ESSENTIALS_${fileNo}.pdf`);
  };

  const panData = result?.pan_data;
