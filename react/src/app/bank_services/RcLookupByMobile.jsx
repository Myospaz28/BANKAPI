...
export default function RcLookupByMobile() {
  ...
  const handleFetch = async () => {
    ...
    /* ===== VALIDATION (SAME STYLE AS RC LITE) ===== */
    if (!mobile || mobile.length !== 10 || !fileNo || !consent) {
      swal.fire({
        title: "Validation Error",
        html: `
          <ul style="text-align:left">
            ${!mobile || mobile.length !== 10 ? "<li>Valid 10-digit Mobile Number is required</li>" : ""}
    ...
    try {
      ...
      const checkRes = await api.post(
        "api/checkRcLookupByMobileCache",
      ...
      if (code === "1000") {
        ...
        swal.fire("No Record Found", apiData?.data?.message, "warning");
      } 
      else {
      ...
      content: [
...

        {
          table: {
            widths: ["40%", "60%"],
            body: [
              [{ text: "Code", bold: true }, safe(result?.data?.code)],
              [{ text: "Message", bold: true }, safe(result?.data?.message)],
              [
                { text: "Mobile Number", bold: true },
                safe(result?.data?.vehicle_data?.mobile_number),
              ],
              [
                { text: "RC Number", bold: true },
                safe(result?.data?.vehicle_data?.rc_number),
              ],
            ],
          },
          layout: "lightHorizontalLines",
