...
export default function PanLookupByMobile() {
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      const checkRes = await api.post(
        "api/checkPanLookupByMobileCache",
        {
          mas_ser_id,
          mas_cat_id,
          mobile_number: mobile,
      ...
      if (code === "1003") {
        ...
        swal.fire("No PAN Found", apiData?.data?.message, "warning");
      } else {
        swal.fire(
          "Completed",
  ...
  const exportPdf = () => {
    ...
    const pan = result?.data?.pan_data || {};
    ...
    const doc = {
      content: [
        { text: "PAN Lookup Detailed Report", style: "header" },
        ...
        { text: "PAN Details", style: "section", margin: [0, 12, 0, 6] },
        {
          table: {
            widths: ["40%", "60%"],
            body: [
              [{ text: "PAN Number", bold: true }, safe(pan.pan_number)],
              [{ text: "Full Name", bold: true }, safe(pan.full_name)],
            ],
          },
          layout: "lightHorizontalLines",
        },
